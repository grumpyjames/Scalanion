...
object ActionPhaseSpec extends Specification with Mockito with CardFilters {
  ...
  trait TestState extends Scope {
    ...
    def expectChoiceAndPlay(actionCards: Seq[Card], actionCard: ActionCard, table: Table, result: ActionResult) {
    ...
    def expectChoiceAndPlay(stacks: Stacks, actionCard: ActionCard, table: Table, result: ActionResult) {
  ...
  "the action phase" should {
    ...
    "skip itself if there are no action cards in the hand" in new TestState {
      val actionResult = runAction(noActionStacks, table)
      there was no(player).chooseFrom(any[Seq[Card]], any[Verb], any[Int], any[Int])
    }

    "offer the player all their remaining actions and then play the selected one" in new TestState {
    ...
    "be ok with the player choosing not to play an action" in new TestState {
    ...
    "discard the played action card" in new TestState {
      expectChoiceAndPlay(actionCards, actionCard, table, boringResult)
      val actionResult = runAction(stacks, table)
      actionResult.stacks.discard.head must_==actionCard
    }

    "transmit the played card to the table" in new TestState {
    ...
    "gracefully deals with opportunities to play more than one action" in new TestState {
    ...
    "can play three actions consecutively" in new TestState {
    ...
    "aggregates buys across multiple action results" in new TestState {
      verifyChoiceAndPlay(twoActionHandStack, actionCard, table, oneBuyResult, verifySecondChoiceThenPlayAction)
    }

    "aggregates treasure across multiple action results" in new TestState {
