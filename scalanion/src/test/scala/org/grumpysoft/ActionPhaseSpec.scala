...
object ActionPhaseSpec extends Specification with Mockito {
  ...
  "the action phase" should {
    ...
    "offer the player all their remaining actions and then play the selected one" in {
      ...
      ActionPhase(1, stacks, player, actionCards)
      there was one(actionCard).play(stacks, player)
    }
...
object ActionPhase {
  ...
  def apply(actionCount: Int, stacks: Stacks, player: GenericPlayer[Card], actionCards: Seq[ActionCard]) : Unit = {
    val chosen = player.chooseFrom(actionCards, Play, 0, 1)
    chosen.headOption match {
      case Some(a) => a match {
        case b : ActionCard => b.play(stacks, player)
